...
async def test_add_cookie(setup_playground):
    ...
    async with ClientSession() as session_http:
        assert (
            await asynchronous.add_cookie(driver.server_url, driver.session, cookie, session_http)
            is True
        )
    cookies_after = synchronous.get_cookies(driver.server_url, driver.session)
    assert len(cookies_after) > len(cookies_before)
...
async def test_switch_to_frame_asynchronous(setup_playground):
    ...
    element_form = synchronous.find_element(
        driver.server_url, driver.session, locator_type_form, locator_form
    )
    synchronous.actions_scroll_to_element(
        driver.server_url, driver.session, element_form, delta_y=1000
    )
...
@mark.asyncio
async def test_switch_to_frame_asynchronous_with_session_http(setup_playground):
    driver = setup_playground
    locator_type = By.ID
    locator_value = "my-iframe"
    locator_type_form = "css selector"
...
@mark.asyncio
async def test_accept_alert_with_session_http(setup_playground):
    driver = setup_playground
    locator_type = By.CSS_SELECTOR
    locator_value = "#alert-button"
...
@mark.asyncio
async def test_dismiss_alert_with_session_http(setup_playground):
    driver = setup_playground
    locator_type = By.CSS_SELECTOR
    locator_value = "#alert-button"
...
async def test_get_tag_name(setup_playground):
    ...
    async with ClientSession() as session_http:
        assert (
            await asynchronous.get_tag_name(
                driver.server_url, driver.session, element, session_http=session_http
            )
            == expected
        )
...
async def test_find_element_from_shadow_root(setup_playground, locator, value):
    ...
    actual = synchronous.get_text(driver.server_url, driver.session, shadow_element)
    assert actual == expected

    async with ClientSession() as session_http:
        shadow_element = await asynchronous.get_shadow_element(
            driver.server_url, driver.session, shadow_root, locator, value, session_http
        )
        actual = synchronous.get_text(driver.server_url, driver.session, shadow_element)
        assert actual == expected
...
async def test_find_elements_from_shadow_root(setup_playground, locator, value):
    ...
    actual = synchronous.get_text(driver.server_url, driver.session, shadow_element[0])
...
async def test_get_rect(setup_playground):
    ...
    actual = synchronous.get_rect(driver.server_url, driver.session, element)
    assert actual["height"]
    assert actual["width"]
    assert actual["x"]
    assert actual["y"]

    actual = await asynchronous.get_rect(driver.server_url, driver.session, element)
    assert actual["height"]
    assert actual["width"]
    assert actual["x"]
    assert actual["y"]
    assert synchronous.get_rect(driver.server_url, driver.session, element)
    async with ClientSession() as session_http:
        actual = await asynchronous.get_rect(
            driver.server_url, driver.session, element, session_http=session_http
        )
        assert actual["height"]
        assert actual["width"]
...
def test_actions_scroll_to_element(setup_playground):
    driver = setup_playground
    locator_type = By.XPATH
    locator_value = "//button"
...
@mark.asyncio
async def test_actions_scroll_to_element_with_session_http(setup_playground):
    driver = setup_playground
    locator_type = By.XPATH
    locator_value = "//button"
...
@mark.asyncio
async def test_submit_foo(setup_playground):
    driver = setup_playground
...
async def test_set_timeouts(setup_playground):
    ...
    async with ClientSession() as session_http:
        await asynchronous.set_timeouts(
            driver.server_url, driver.session, timeouts_2, session_http=session_http
        )
...
@mark.asyncio
async def test_execute_script_asynchronous_with_session_http(setup_playground):
    driver = setup_playground
    script = "alert('any warn')"
    async with ClientSession() as session_http:
        assert (
...
async def test_get_alert_text(setup_playground):
    ...
    async with ClientSession() as session_http:
        assert (
            await asynchronous.get_alert_text(
                driver.server_url, driver.session, session_http=session_http
            )
            == expected
        )
...
async def test_get_active_element(setup_playground):
    ...
    async with ClientSession() as session_http:
        assert (
            await asynchronous.get_active_element(
                driver.server_url, driver.session, session_http=session_http
            )
            == element
        )
...
async def test_get_css_value(setup_playground):
    ...
    assert expected in synchronous.get_css_value(
        driver.server_url, driver.session, element, property_name
    )
...
@mark.asyncio
async def test_close_window_async_with_session_http(setup_playground):
    driver = setup_playground   
    async with ClientSession() as session_http:
        response = await asynchronous.close_window(
            driver.server_url, driver.session, session_http=session_http
        )
    assert isinstance(response, list)
...
async def test_get_attribute(setup_playground):
    ...
    assert expected in synchronous.get_attribute(
        driver.server_url, driver.session, element, attribute
    )
    assert expected in await asynchronous.get_attribute(
        driver.server_url, driver.session, element, attribute
    )
    async with ClientSession() as session_http:
        assert expected in await asynchronous.get_attribute(
...
async def test_get_cookies(setup_playground):
    ...
    async with ClientSession() as session_http:
        cookies = await asynchronous.get_cookies(
            driver.server_url, driver.session, session_http=session_http
        )
    assert isinstance(cookies, list)
...
async def test_go_back(setup_playground):
    ...
    synchronous.get(driver.server_url, driver.session, OTHER_URL)
    assert synchronous.go_back(driver.server_url, driver.session) is True
    assert synchronous.get_title(driver.server_url, driver.session) == title_sample

    synchronous.go_forward(driver.server_url, driver.session) is True
    assert synchronous.get_title(driver.server_url, driver.session) == title_other
    async with ClientSession() as session_http:
        assert (
            await asynchronous.go_back(driver.server_url, driver.session, session_http=session_http)
            is True
        )
    assert synchronous.get_title(driver.server_url, driver.session) == title_sample
