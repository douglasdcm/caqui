...
class TestCDPFunctions:
    ...
    @mark.asyncio
    async def test_cdp_get_title_from_blank_page(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_get_body_from_blank_page(self, setup_env):
        await asynchronous.new_window(setup_env)
        handle = (await asynchronous.get_window_handles(setup_env))[1]
        new_conn = await asynchronous.switch_to_window(setup_env, handle)
    ...
    @mark.asyncio
    async def test_cdp_get_shadow_element(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_get_shadow_elements(self, setup_env):
        shadow_element = await asynchronous.find_element(setup_env, By.ID, "shadow-root")
        element = (
    ...
    @mark.asyncio
    async def test_cdp_find_elements(self, setup_env, locator: Tuple):
        assert await asynchronous.find_elements(setup_env, *locator) is not None

    @mark.asyncio
    async def test_cdp_click_foo(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_send_keys(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_get_text(self, setup_env):
        expected = "Basic page"
    ...
    @mark.asyncio
    async def test_cdp_get_attribute_style(self, setup_env):
        expected = "color: red;"
    ...
    @mark.asyncio
    async def test_cdp_get_attribute_href(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_element_is_not_selected(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_get_css_value(self, setup_env):
        expected = "red"
        element = await asynchronous.find_element(setup_env, By.ID, "button")
        await asynchronous.click(setup_env, element)
    ...
    @mark.asyncio
    async def test_cdp_handle_cookie(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_get_cookies(self, setup_env):
        await asynchronous.get(setup_env, "https://example.org/")
        await asynchronous.add_cookie(setup_env, COOKIE)
    ...
    @mark.asyncio
    async def test_cdp_delete_all_cookies(self, setup_env):
        await asynchronous.get(setup_env, "https://example.org/")
        await asynchronous.add_cookie(setup_env, COOKIE)
        assert (
    ...
    @mark.asyncio
    async def test_cdp_switch_to_frame(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_switch_to_parent_frame(self, setup_env):
        iframe_id = await asynchronous.find_element(setup_env, By.ID, "my-iframe")
        iframe_node = await asynchronous.switch_to_frame(setup_env, iframe_id)
    ...
    @mark.asyncio
    async def test_cdp_send_alert_text(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_send_alert_text_fails_if_no_alert(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_accept_alert(self, setup_env):
        alert = await asynchronous.find_element(setup_env, By.ID, "alert-button-prompt")
        assert await asynchronous.accept_alert(setup_env, alert) is None

    @mark.asyncio
    async def test_cdp_accept_alert_fails_if_no_alert(self, setup_env):
        no_alert = await asynchronous.find_element(setup_env, By.ID, "button")
        with raises(WebDriverError):
            assert await asynchronous.accept_alert(setup_env, no_alert, 0.1) is None

    @mark.asyncio
    async def test_cdp_dismiss_alert(self, setup_env):
        alert = await asynchronous.find_element(setup_env, By.ID, "alert-button-prompt")
        assert await asynchronous.dismiss_alert(setup_env, alert) is None

    @mark.asyncio
    async def test_cdp_dismiss_alert_fails_if_no_alert(self, setup_env):
        no_alert = await asynchronous.find_element(setup_env, By.ID, "button")
        with raises(WebDriverError):
            assert await asynchronous.dismiss_alert(setup_env, no_alert, 0.1) is None

    @mark.asyncio
    async def test_cdp_get_alert_text(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_take_screenshot_of_element(self, setup_env):
        element = await asynchronous.find_element(setup_env, By.ID, "button")
        assert await asynchronous.take_screenshot_element(setup_env, element) is None

    @mark.asyncio
    async def test_cdp_take_screenshot_of_page(self, setup_env):
        assert await asynchronous.take_screenshot(setup_env) is None

    @mark.asyncio
    async def test_cdp_get_computed_label(self, setup_env):
        element = await asynchronous.find_element(setup_env, By.ID, "button")
        assert await asynchronous.get_computed_label(setup_env, element) == "test"

    @mark.asyncio
    async def test_cdp_get_computed_role(self, setup_env):
        element = await asynchronous.find_element(setup_env, By.ID, "button")
        assert await asynchronous.get_computed_role(setup_env, element) == "button"

    @mark.asyncio
    async def test_cdp_get_tag_name(self, setup_env):
        element = await asynchronous.find_element(setup_env, By.ID, "button")
        assert await asynchronous.get_tag_name(setup_env, element) == "BUTTON"

    @mark.asyncio
    async def test_cdp_get_rect(self, setup_env):
        element = await asynchronous.find_element(setup_env, By.ID, "button")
    ...
    @mark.asyncio
    async def test_cdp_actions_move_to_element(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_actions_scroll_to_element(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_submit_form(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_find_children_elements(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_find_child_element(self, setup_env, locator: Tuple):
    ...
    @mark.asyncio
    async def test_cdp_get_page_source(self, setup_env):
        assert "body" in await asynchronous.get_page_source(setup_env)

    @mark.asyncio
    async def test_cdp_execute_script(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_get_active_element(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_clear_element(self, setup_env):
        element = await asynchronous.find_element(setup_env, By.XPATH, "//input")
        await asynchronous.send_keys(setup_env, element, "any")
        assert await asynchronous.clear_element(setup_env, element) is None

    @mark.asyncio
    async def test_cdp_is_enabled(self, setup_env):
    ...
    @mark.asyncio
    async def test_cdp_get_window_rectangle(self, setup_env):
    ...
    # @mark.asyncio
    # async def test_cdp_close_window(self, setup_env):
    #
    #         await asynchronous.new_window(setup_env)
    #         assert await asynchronous.close_window(setup_env) is None

    @mark.asyncio
    async def test_cdp_get_window_handler(self, setup_env):
        assert await asynchronous.get_window(setup_env) is not None

    @mark.asyncio
    async def test_cdp_get_status(self, setup_env):
