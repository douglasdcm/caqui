...
async def test_delete_cookie_asynchronous(setup_functional_environment):
    ...
    cookie = COOKIE
    synchronous.add_cookie(server_url, session, cookie)
...
async def test_switch_to_window(setup_functional_environment, window_type):
    ...
    new_page = handles[1]
    assert synchronous.switch_to_window_jsonwire(server_url, session, handle=new_page) is True
...
async def test_switch_to_frame_asynchronous(setup_functional_environment):
    ...
    locator_value = "my-iframe"
    element_frame = synchronous.find_element(server_url, session, locator_type, locator_value)
    assert await asynchronous.switch_to_frame_jsonwire(server_url, session, element_frame) is True
...
def test_switch_to_frame_synchronous(setup_functional_environment):
    ...
    locator_value = "my-iframe"
    element_frame = synchronous.find_element(server_url, session, locator_type, locator_value)
    assert synchronous.switch_to_frame_jsonwire(server_url, session, element_frame) is True
...
async def test_get_named_cookie(setup_functional_environment):
    ...
    cookie = COOKIE
    synchronous.add_cookie(server_url, session, cookie)
    name = "NID"  # cookie created on page load
    expected = ".example.org"
    assert synchronous.get_named_cookie(server_url, session, name).get("domain") == expected
    response = await asynchronous.get_named_cookie(server_url, session, name)
    assert response.get("domain") == expected
...
async def test_get_rect(setup_functional_environment):
    ...
    locator_type = By.XPATH
    locator_value = "//input"
    expected = {"height": 35, "width": 745, "x": 20, "y": 175.8125}
    element = synchronous.find_element(server_url, session, locator_type, locator_value)
...
async def test_move_to_element(setup_functional_environment):
    ...
    element = synchronous.find_element(server_url, session, locator_type, locator_value)
    assert synchronous.actions_move_to_element_jsonwire(server_url, session, element) is True
    assert await asynchronous.actions_move_to_element_jsonwire(server_url, session, element) is True
...
async def test_actions_click(setup_functional_environment):
    ...
    locator_value = "//button"
    element = synchronous.find_element(server_url, session, locator_type, locator_value)
    assert synchronous.actions_click_jsonwire(server_url, session, element) is True
    assert await asynchronous.actions_click_jsonwire(server_url, session, element) is True
