...
from caqui.cdp.connection import CDPConnection, CDPConnectionSync
...
class TestCDPSyncFunctions:
    @pytest.fixture
    def setup_env(self):
        conn = CDPConnectionSync(get_ws_url())
        conn.connect()
    ...
    def test_cdp_sync_get_title_from_blank_page(self, setup_env):
        ...
        handle = (synchronous.get_window_handles(setup_env))[1]
    ...
    def test_cdp_sync_get_shadow_element(self, setup_env):
        ...
        element = synchronous.get_shadow_element(
    ...
    def test_cdp_sync_get_shadow_elements(self, setup_env):
        shadow_element = synchronous.find_element(setup_env, By.ID, "shadow-root")
        element = (
            synchronous.get_shadow_elements(
                setup_env, shadow_element, By.ID, "shadow-button"
            )
        )[0]
        assert synchronous.get_tag_name(setup_env, element) == "BUTTON"
