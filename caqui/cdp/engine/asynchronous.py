...
class GlobalValues:
    ...
    async def get_document_node(refresh=False) -> dom.Node:
        ...
        if not GlobalValues._doc:
            GlobalValues._doc = await GlobalValues.conn.execute(
                dom.get_document(depth=-1, pierce=True)
            )
        return GlobalValues._doc


async def _get_element_center(conn, element):
...
async def _find_element_by_xpath(
    ...
    if root_id:
        remote_root = await conn.execute(dom.resolve_node(node_id=root_id))
    else:
        node = await GlobalValues.get_document_node()
        remote_root = await conn.execute(dom.resolve_node(node_id=node.node_id))
    object_id = remote_root.object_id
    call_res = await conn.execute(
    ...
    node = await conn.execute(dom.request_node(object_id=remote.object_id))
    if node == 0:
...
async def _find_all_elements_by_xpath(
    ...
    for prop in properties[0]:
        ...
        node_id = await conn.execute(dom.request_node(object_id=remote_node.object_id))
...
async def _find_element(
    ...
    if locator_type == By.XPATH:
        try:
            return await _find_element_by_xpath(conn, selector, root_id)
        except Exception:
            raise
    if locator_type == By.CSS_SELECTOR:
        return await _find_element_by_css_selector(conn, root_id, depth, pierce, selector)
...
async def _find_all_elements(
    ...
    if locator_type == By.XPATH:
        return await _find_all_elements_by_xpath(conn, locator_value, root_id)
    if locator_type == By.CSS_SELECTOR:
        if root_id:
            node = await conn.execute(dom.describe_node(root_id))
        else:
...
async def _refresh_agents(conn: AsyncCDPConnection):
    ...
    await GlobalValues.get_document_node(refresh=True)


async def go_to_page(conn, page_url: str) -> None:
    """Navigate to 'page_url'"""
    await get(conn, page_url)
...
async def find_element(
    ...
    """
    try:
        return await _find_element(conn, locator_type, locator_value, root_id)
    except Exception as e:
...
async def get_title(conn: AsyncCDPConnection) -> str:
    ...
    try:
        ...
        pass
    try:
        root_node = await GlobalValues.get_document_node()
...
async def switch_to_window(conn: AsyncCDPConnection, handle: target.TargetInfo):
    ...
    Returns:
        new_conn (CDPConnection): The new connection to the websocket.
    ...
    try:
        handles = await get_window_handles(conn)
        index = 0
        for h in handles:
        ...
        with urllib.request.urlopen(f"http://127.0.0.1:{port}/json") as r:
            ...
            await conn.set_url(ws_url)
            await conn.connect()
            new_conn = conn
            await _refresh_agents(new_conn)
            return new_conn
    except Exception as e:
        raise WebDriverError("Failed to switch to window.") from e


async def new_window(conn: AsyncCDPConnection) -> None:
    """
    Open a new window.
...
async def switch_to_frame(conn: AsyncCDPConnection, element_frame: dom.NodeId) -> dom.NodeId:
    """
...
async def get_named_cookie(conn, name) -> dict:
    ...
    """
    try:
        cookies = await get_cookies(conn)
...
async def get_computed_role(conn: AsyncCDPConnection, element: dom.NodeId) -> str:
    ...
    try:
        ...
        ax_nodes_response = await conn.execute(
...
async def get_shadow_element(
    ...
    """
    try:
        host_details = await conn.execute(dom.describe_node(node_id=shadow_element, depth=1))
        shadow_roots = host_details.shadow_roots
        return await _find_element(conn, locator_type, locator_value, shadow_roots[0].node_id)
    except Exception as e:
        raise WebDriverError("Failed to get the element shadow.") from e
...
async def get_shadow_elements(
    ...
    try:
        host_details = await conn.execute(dom.describe_node(node_id=shadow_element, depth=1))
        shadow_roots = host_details.shadow_roots
        return await _find_all_elements(conn, locator_type, locator_value, shadow_roots[0].node_id)
    except Exception as e:
        raise WebDriverError("Failed to get the element shadow.") from e
...
async def actions_move_to_element(conn: AsyncCDPConnection, element: dom.NodeId) -> None:
    """
    ...
    """Move to an element simulating a mouse movement"""
    try:
        x, y = await _get_element_center(conn, element)
        await conn.execute(
            input_.dispatch_mouse_event(
                "mouseMoved",
                x=x,
...
async def find_children_elements(
    ...
    """
    try:
        return await _find_all_elements(conn, locator_type, locator_value, parent_element)
    except Exception as e:
        raise WebDriverError(
...
async def find_child_element(
    ...
    try:
        return await find_element(conn, locator_type, locator_value, parent_element)
    except Exception as e:
...
async def is_element_enabled(conn: AsyncCDPConnection, element) -> bool:
    ...
    try:
        script = "disabled"
        result = await execute_script(conn, element, script, positive=False, return_by_value=True)
...
async def close_window(conn: AsyncCDPConnection):
    ...
    try:
        ...
        await conn.execute(target.page.close())
    except Exception as e:
...
async def get_window(conn) -> str:
    """Get window"""
    try:
        result = await get_window_handles(conn)
